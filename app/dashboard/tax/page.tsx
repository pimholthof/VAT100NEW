...
export default function TaxPage() {
  ...
  return (
    <div>
      ...
      <div className="page-header" style={{ marginBottom: 80 }}>
        <div>
          ...
          <p style={{ fontSize: "var(--text-body-lg)", fontWeight: 300, margin: "16px 0 0", opacity: 0.5 }}>
            Overzicht van je BTW en inkomstenbelasting
          </p>
        </div>
        <a
          href="/api/export/btw"
      ...
      >
        <YearCard
          label={`Geschatte inkomstenbelasting ${now.getFullYear()}`}
          value={formatCurrency(Math.round(estimatedIB))}
          sublabel={`Belastbaar inkomen: ${formatCurrency(Math.round(taxableProfit))}`}
        />
        <YearCard
        ...
        />
        <YearCard
          label="Totaal aan belasting"
          value={formatCurrency(Math.round(estimatedIB + Math.max(0, yearBtw)))}
          sublabel={safeToSpend ? `Vrij te besteden: ${formatCurrency(safeToSpend.safeToSpend)}` : ""}
        />
      </div>
      ...
      <div style={{ marginBottom: "var(--space-section)" }}>
        ...
        <div className="responsive-grid-3">
          ...
          <DeductionItem label="MKB-winstvrijstelling" value={formatCurrency(Math.round(mkbVrijstelling))} note="14% van je winst" />
          <DeductionItem
            label="Terug te vragen BTW (dit jaar)"
      ...
      {!isLoading && current && (
        <div style={{ marginBottom: "var(--space-section)" }}>
          <p className="label" style={{ margin: "0 0 16px", opacity: 0.3 }}>
            {current.netVat >= 0 ? "Dit moet je betalen" : "Dit krijg je terug"}
          </p>
      ...
      <div style={{ padding: 20, background: "rgba(13,13,11,0.02)", fontSize: 11, fontWeight: 400, lineHeight: 1.6 }}>
        Dit overzicht is indicatief. Doe je BTW-aangifte via het portaal van
        de Belastingdienst. Bewaar je facturen en bonnen minimaal 7 jaar.
        Schatting inkomstenbelasting is gebaseerd op 36,93% schijf 1 en 49,50% schijf 2 (2024).
