...
export default async function LeadDetailPage({ params }: LeadDetailPageProps) {
  ...
  const [activitiesResult, initialTasksResult] = await Promise.all([
  ...
  const activities = (activitiesResult.data || []) as { id: string; created_at: string; activity_type: string; description: string }[];
  const tasks = (tasksResult.data || []) as { id: string; title: string; completed: boolean; completed_at: string | null }[];
