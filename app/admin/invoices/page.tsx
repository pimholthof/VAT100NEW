  ...
  if (result?.error) {
    return (
      <div className="admin-layout">
        <PageHeader title="Facturen" backHref="/admin" backLabel="Command Center" />
        <AdminStatePanel eyebrow="Facturen" title="Kon niet worden geladen" description={result.error} actions={[{ href: "/admin", label: "Terug", variant: "secondary" }]} />
      </div>
    );
  }
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id}>
                    ...
                    <td>
                      <Link href={`/admin/users/${inv.user_id}`} className="admin-primary-link">
                        {inv.user_name}
                      </Link>
                    </td>
