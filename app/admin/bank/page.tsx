  ...
  if (result?.error) {
    return (
      <div className="admin-layout">
        <PageHeader title="Bankkoppelingen" backHref="/admin" backLabel="Command Center" />
        <AdminStatePanel eyebrow="Bank" title="Kon niet worden geladen" description={result.error} actions={[{ href: "/admin", label: "Terug", variant: "secondary" }]} />
      </div>
    );
  }
            ...
            <table className="admin-table">
              ...
              <tbody>
                {connections.map((conn) => (
                  <tr key={conn.id}>
                    <td>
                      <Link href={`/admin/users/${conn.user_id}`} className="admin-primary-link">
                        {conn.user_name}
                      </Link>
                    </td>
