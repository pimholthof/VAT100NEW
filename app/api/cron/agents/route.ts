...
export async function GET(request: NextRequest) {
  ...
  // Initiële call: bescherm met cron lock om dubbele runs te voorkomen
  if (isInitialCall) {
    const locked = await withCronLock("agents", runAgentBatch, 30);
    if (locked === null) {
  ...
  // Continuation call: draait binnen bestaande cron-run
