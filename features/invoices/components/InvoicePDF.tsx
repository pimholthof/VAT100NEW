...
const s5 = StyleSheet.create({
  ...
  block: { width: "48%" },
  metaBlock: { width: "48%" },
  name: { fontSize: 10, color: INK5, marginBottom: 8 },
  line: { fontSize: 9, color: INK5, lineHeight: 1.55 },
  metaPair: { flexDirection: "row", justifyContent: "space-between", marginBottom: 4 },
  metaLbl: { fontSize: 9, color: INK5 },
  metaVal: { fontSize: 9, color: INK5 },
  cli: { marginTop: 18, marginBottom: 18 },
  cliLbl: { fontSize: 9, color: INK5, marginBottom: 2 },
  cliLine: { fontSize: 9, color: INK5, lineHeight: 1.55 },
  obj: { marginTop: 18, marginBottom: 26 },
  objLbl: { fontSize: 9, color: INK5, marginBottom: 2 },
  objBody: { fontSize: 9, color: INK5, lineHeight: 1.55 },
  thead: { flexDirection: "row", borderBottomWidth: 0.5, borderBottomColor: INK5, paddingBottom: 6, marginBottom: 8 },
  ...
  footRight: { fontSize: 6.5, color: INK5 },
});

function EditoriaalPDF({ data, locale, branded }: { data: InvoiceData; locale: Locale; branded: boolean }) {
  const t = getDictionary(locale);
  const { invoice, lines, client, profile } = data;
  const cr = invoice.is_credit_note;
  const days = calculatePaymentDays({ issueDate: invoice.issue_date, dueDate: invoice.due_date, defaultDays: 30 });
