...
function BillboardHTML({ data, locale }: { data: InvoiceData; locale: Locale }) {
  ...
  const lbl: React.CSSProperties = { fontSize: 9, letterSpacing: "0.3em", color: GREY, textTransform: "uppercase", fontWeight: 400 };
  const partyLine: React.CSSProperties = { fontSize: 10, color: INK, lineHeight: 1.7 };
  const partyDetail: React.CSSProperties = { fontSize: 9, color: GREY, lineHeight: 1.7, marginTop: 4 };

  return (
    <div style={{ width: 595, minHeight: 842, padding: "56px 56px 56px", fontFamily: F, color: INK, background: BG, position: "relative", boxSizing: "border-box" }}>
      {/* Header: VAT100 logo left, date/nr right */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div style={{ fontSize: 52, fontWeight: 700, letterSpacing: "-0.04em", lineHeight: 0.9 }}>
          VAT100
        </div>
        <div style={{ textAlign: "right", paddingTop: 12 }}>
          <div style={{ fontSize: 10, color: INK, marginBottom: 3 }}>{fmtDateLong(invoice.issue_date, locale)}</div>
          <div style={{ fontSize: 10, color: GREY }}>Nr. {invoice.invoice_number}</div>
        </div>
      </div>
      <div style={{ ...lbl, marginBottom: 40 }}>
        {isCreditNote ? t.invoiceDoc.creditNote : t.invoiceDoc.invoice}
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
        <div style={{ width: 240 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 0", marginTop: 6, borderTop: `1px solid ${INK}` }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.3em", textTransform: "uppercase" }}>{t.invoiceDoc.total}</span>
            <span style={{ fontSize: 18, fontWeight: 700 }}>{fmtEur(invoice.total_inc_vat)}</span>
          </div>
        </div>
      </div>
      ...
      {invoice.due_date && (
        <div style={{ marginTop: 32 }}>
          <div style={{ fontSize: 9, color: GREY }}>{t.invoiceDoc.paymentTerms}: {fmtDateLong(invoice.due_date, locale)}</div>
        </div>
      )}
      ...
      <div style={{ position: "absolute", bottom: 56, left: 56, right: 56, borderTop: `0.5px solid ${RULE}`, paddingTop: 14, display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 28 }}>
          {profile.iban && <div><div style={{ ...lbl, fontSize: 7, marginBottom: 3 }}>IBAN</div><div style={{ fontSize: 8.5 }}>{profile.iban}</div></div>}
          {profile.bic && <div><div style={{ ...lbl, fontSize: 7, marginBottom: 3 }}>BIC</div><div style={{ fontSize: 8.5 }}>{profile.bic}</div></div>}
          <div><div style={{ ...lbl, fontSize: 7, marginBottom: 3 }}>{t.invoiceDoc.paymentTerms}</div><div style={{ fontSize: 8.5 }}>{days} {t.invoiceDoc.daysNet}</div></div>
        </div>
