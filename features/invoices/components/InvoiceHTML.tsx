...
export function InvoiceHTML({ data, template = "minimaal" }: { data: InvoiceData; template?: InvoiceTemplate }) {
  switch (template) {
    case "klassiek": return <KlassiekHTML data={data} />;
    case "strak": return <StrakHTML data={data} />;
    default: return <MinimaalHTML data={data} />;
  }
}
...
function MinimaalHTML({ data }: { data: InvoiceData }) {
      ...
      <div style={{ display: "flex", marginBottom: 28 }}>
        ...
        <div style={{ width: COL_R }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
            <span style={lbl}>Datum</span><span style={{ fontSize: 9 }}>{fmtDate(invoice.issue_date)}</span>
          </div>
          {invoice.due_date && (
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
              <span style={lbl}>Vervaldatum</span><span style={{ fontSize: 9 }}>{fmtDate(invoice.due_date)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}>
        <div style={{ width: COL_R }}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}>
            <span style={lbl}>Subtotaal</span><span style={{ fontSize: 9, color: GREY }}>{fmtEur(invoice.subtotal_ex_vat)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}>
            <span style={lbl}>BTW {invoice.vat_rate ?? 21}%</span><span style={{ fontSize: 9, color: GREY }}>{fmtEur(invoice.vat_amount)}</span>
          </div>
...
function KlassiekHTML({ data }: { data: InvoiceData }) {
  const { invoice, lines, client, profile } = data;
  const isCreditNote = invoice.is_credit_note;
  const days = calculatePaymentDays({ issueDate: invoice.issue_date, dueDate: invoice.due_date, defaultDays: 30 });
  const showContact = client.contact_name && client.contact_name.toLowerCase() !== client.name.toLowerCase();
  ...
  return (
    <div style={{ width: 595, minHeight: 842, padding: "56px 48px 48px", fontFamily: F, color: INK, background: "#FAFAF8", position: "relative", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", gap: 40, marginBottom: 20 }}>
        <div style={{ flex: 1 }}>
          <div style={{ ...lbl, marginBottom: 8 }}>Van</div>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ ...lbl, marginBottom: 8 }}>Aan</div>
          ...
          {client.kvk_number && <div style={{ ...bodyGrey, marginTop: 4 }}>KVK {client.kvk_number}</div>}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", borderBottom: `1px solid ${INK}`, paddingBottom: 8 }}>
          <div style={{ ...lbl, width: "46%" }}>Omschrijving</div>
          <div style={{ ...lbl, width: "16%", textAlign: "center" }}>Aantal</div>
          <div style={{ ...lbl, width: "18%", textAlign: "right" }}>Tarief</div>
          <div style={{ ...lbl, width: "20%", textAlign: "right" }}>Bedrag</div>
        </div>
        {lines.map((l, i) => (
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div style={{ width: 240 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 0", marginTop: 6, borderTop: `1.5px solid ${INK}` }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>Totaal</span>
      ...
      {invoice.due_date && (
        <div style={{ marginTop: 40 }}>
          <div style={{ fontSize: 8.5, color: GREY }}>Gelieve te betalen vóór {fmtDateLong(invoice.due_date)}</div>
        </div>
      )}
      ...
      <div style={{ position: "absolute", bottom: 48, left: 48, right: 48, borderTop: `0.5px solid ${RULE}`, paddingTop: 12, display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 32 }}>
          ...
          <div><div style={{ ...lbl, fontSize: 6.5, marginBottom: 2 }}>Betaaltermijn</div><div style={{ fontSize: 8.5, fontWeight: 500 }}>{days} dagen</div></div>
        </div>
...
function StrakHTML({ data }: { data: InvoiceData }) {
  const { invoice, lines, client, profile } = data;
  const isCreditNote = invoice.is_credit_note;
  const days = calculatePaymentDays({ issueDate: invoice.issue_date, dueDate: invoice.due_date, defaultDays: 30 });
  const showContact = client.contact_name && client.contact_name.toLowerCase() !== client.name.toLowerCase();
  ...
  return (
    <div style={{ width: 595, minHeight: 842, padding: "64px 56px 56px", fontFamily: F, color: INK, background: "#FAF9F6", position: "relative", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 48, paddingBottom: 16, borderBottom: `0.5px solid ${RULE}` }}>
        <div>
          <div style={{ ...lbl, marginBottom: 6 }}>{isCreditNote ? "Creditnota" : "Factuur"}</div>
        ...
        {invoice.due_date && (
          <div style={{ textAlign: "right" }}>
            <div style={{ ...lbl, marginBottom: 6 }}>Vervaldatum</div>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", paddingBottom: 8, borderBottom: `0.5px solid ${RULE}` }}>
          <div style={{ ...lbl, width: "50%" }}>Omschrijving</div>
          <div style={{ ...lbl, width: "14%", textAlign: "right" }}>Aantal</div>
          <div style={{ ...lbl, width: "18%", textAlign: "right" }}>Tarief</div>
          <div style={{ ...lbl, width: "18%", textAlign: "right" }}>Bedrag</div>
        </div>
        {lines.map((l) => (
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div style={{ width: 200 }}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0" }}>
            <span style={lbl}>Subtotaal</span><span style={{ fontSize: 8, color: GREY }}>{fmtEur(invoice.subtotal_ex_vat)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0" }}>
            <span style={lbl}>BTW {invoice.vat_rate ?? 21}%</span><span style={{ fontSize: 8, color: GREY }}>{fmtEur(invoice.vat_amount)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0 0", marginTop: 8, borderTop: `0.5px solid ${INK}` }}>
            <span style={{ ...lbl, color: INK, fontWeight: 600 }}>Totaal</span>
      ...
      <div style={{ position: "absolute", bottom: 56, left: 56, right: 56 }}>
        <div style={{ borderTop: `0.5px solid ${RULE}`, paddingTop: 12, display: "flex", gap: 40 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 24 }}>
            ...
            <div><div style={{ ...lbl, fontSize: 6, marginBottom: 2 }}>Betaaltermijn</div><div style={{ fontSize: 7.5 }}>{days} dagen</div></div>
          </div>
        </div>
      </div>
    </div>
