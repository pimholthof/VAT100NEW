...
export async function getLeadActivities(leadId: string): Promise<ActionResult<any[]>> {
  const auth = await requireAdmin();
  if (auth.error !== null) return { error: auth.error };
...
export async function getLeadTasks(leadId: string): Promise<ActionResult<any[]>> {
  const auth = await requireAdmin();
  if (auth.error !== null) return { error: auth.error };
...
export async function getRevenueMetrics(): Promise<ActionResult<RevenueMetrics>> {
  try {
    ...
    const mrr_cents = subsData?.reduce((acc, sub: any) => acc + (sub.plan?.price_cents || 0), 0) || 0;
    ...
    const pipeline_value_cents = pipelineLeads?.reduce((acc, lead: any) => {
      return acc + (lead.plans?.price_cents || 0);
    }, 0) || 0;
