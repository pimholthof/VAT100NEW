  ...
  run: async (event: SystemEventRow) => {
    ...
    try {
      if (event.event_type === "system.daily_deadline_check") {
        await performDailyDeadlineCheck(supabase, userId);
      } else if (event.event_type === "user.profile_updated") {
        await checkInitialCompliance(supabase, userId);
      } else if (event.event_type === "invoice.created" || event.event_type === "receipt.uploaded") {
        await updateComplianceStatus(supabase, userId);
      }
...
async function performDailyDeadlineCheck(supabase: SupabaseServiceClient, userId: string) {
  const now = new Date();

  // Check BTW deadlines
  await checkVatDeadlines(supabase, userId, now);

  // Check Inkomstenbelasting deadline
  await checkIncomeTaxDeadlines(supabase, userId, now);

  // Check Urencriterium voortgang
  await checkHoursProgress(supabase, userId, now);

  // Check KIA deadline
  await checkKiaDeadline(supabase, userId, now);

  // Check compliance status
  await updateComplianceStatus(supabase, userId);
}

async function checkVatDeadlines(supabase: SupabaseServiceClient, userId: string, now: Date) {
  ...
  if (daysUntilDeadline <= 0) {
    await createVatAlert(supabase, userId, "OVERDUE", currentQuarter, currentYear, 0);
  } else if (daysUntilDeadline <= 7) {
    await createVatAlert(supabase, userId, "URGENT", currentQuarter, currentYear, daysUntilDeadline);
  } else if (daysUntilDeadline <= 21) {
    await createVatAlert(supabase, userId, "WARNING", currentQuarter, currentYear, daysUntilDeadline);
  } else if (daysUntilDeadline <= 60) {
    await createVatAlert(supabase, userId, "REMINDER", currentQuarter, currentYear, daysUntilDeadline);
  }
}

async function checkIncomeTaxDeadlines(supabase: SupabaseServiceClient, userId: string, now: Date) {
  const currentYear = now.getFullYear();
  ...
  if (daysUntilDeadline <= 30 && daysUntilDeadline > 0) {
    await createTaxAlert(supabase, userId, "INCOME_TAX", currentYear - 1, daysUntilDeadline);
  }
}

async function checkHoursProgress(supabase: SupabaseServiceClient, userId: string, now: Date) {
  const currentYear = now.getFullYear();
  ...
  if (totalHours < targetHours * 0.8) {
    ...
    await createHoursAlert(supabase, userId, shortfall, Math.round(totalHours), progressPercentage);
  }
}

async function checkKiaDeadline(supabase: SupabaseServiceClient, userId: string, now: Date) {
  const currentYear = now.getFullYear();
  ...
  if (daysUntilDeadline <= 90 && daysUntilDeadline > 0) {
    ...
    if (totalInvestments < 2400) { // Onder de eerste KIA schijf
      await createKiaAlert(supabase, userId, totalInvestments, daysUntilDeadline);
    }
  }
}
...
async function updateComplianceStatus(supabase: SupabaseServiceClient, userId: string) {
  const now = new Date();
...
async function checkInitialCompliance(supabase: SupabaseServiceClient, userId: string) {
  ...
  if (missingFields.length > 0) {
    await createComplianceAlert(supabase, userId, "MISSING_PROFILE", missingFields);
  }
}
...
async function hasActiveAlert(
  ...
  userId: string,
  alertType: string,
  extraFilters?: { quarter?: number; year?: number }
): Promise<boolean> {
  let query = supabase
  ...
  if (extraFilters?.quarter != null) query = query.eq("quarter", extraFilters.quarter);
  if (extraFilters?.year != null) query = query.eq("year", extraFilters.year);

  const { data } = await query.limit(1);
  return (data?.length ?? 0) > 0;
}

// ─── Alert creation functies (met deduplicatie) ───

async function createVatAlert(supabase: SupabaseServiceClient, userId: string, urgency: string, quarter: number, year: number, daysUntil: number) {
  // Skip als er al een actieve alert bestaat voor dit kwartaal/jaar
  if (await hasActiveAlert(supabase, userId, "vat_deadline", { quarter, year })) return;
...
async function createTaxAlert(supabase: SupabaseServiceClient, userId: string, type: string, year: number, daysUntil: number) {
  if (await hasActiveAlert(supabase, userId, "income_tax_deadline", { year })) return;
...
async function createHoursAlert(supabase: SupabaseServiceClient, userId: string, shortfall: number, currentHours: number, progress: number) {
  if (await hasActiveAlert(supabase, userId, "hours_progress")) return;
...
async function createKiaAlert(supabase: SupabaseServiceClient, userId: string, currentInvestments: number, daysUntil: number) {
  if (await hasActiveAlert(supabase, userId, "kia_deadline")) return;
...
async function createComplianceAlert(supabase: SupabaseServiceClient, userId: string, type: string, missingFields: string[]) {
  if (await hasActiveAlert(supabase, userId, "compliance")) return;
