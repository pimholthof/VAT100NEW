...
- [x] **6d** `features/invoices/actions.test.ts` — nieuw: validatie pipeline tests
- [x] **6e** `features/clients/actions.test.ts` — nieuw: client validatie tests

---

## Sprint 7: Responsive fixes ✓

- [x] **7a** `DashboardNav.tsx` — font scaling consistent maken
- [x] **7b** `Table.tsx` — TableWrapper horizontaal scrollbaar op mobiel
- [x] **7c** Factuur lijst pagina — touch targets 44x44px via .table-action
- [x] **7d** Receipt lijst pagina — touch targets 44x44px via .table-action
...
- [x] **8d** Loading skeleton — al consistent via ListLoading component

---

## Sprint 9: Security quick wins ✓

- [x] **9a** Share token — expiry veld in DB migration
- [x] **9b** Share token — expiry check in `fetch-public.ts`
- [x] **9c** File upload — magic byte check in `ReceiptUpload.tsx`
- [x] **9d** API rate limit — in-memory limiter op `/api/ai/chat`
...
- [x] **10c** Credit nota — visuele markering in lijst (accent label)
- [x] **10d** BTW pagina — kwartaal selector dropdown

---

## Status: COMPLEET ✓

Alle 45 items zijn afgerond in 10 commits.
