...
test.describe('Wachtlijst formulier', () => {
  ...
  test('wachtlijst formulier bevat naam- en e-mailvelden', async ({ page }) => {
    await page.goto('/');
  ...
  test('wachtlijst formulier vereist verplichte velden', async ({ page }) => {
    await page.goto('/');
  ...
  test('wachtlijst formulier verzendt succesvol met geldige gegevens', async ({ page }) => {
    ...
    // Na succesvolle indiening toont het formulier een bevestiging
    await expect(page.getByText('Je staat op de lijst')).toBeVisible({ timeout: 10000 });
  });

  test('wachtlijst verzendknop is aanwezig', async ({ page }) => {
    await page.goto('/');
